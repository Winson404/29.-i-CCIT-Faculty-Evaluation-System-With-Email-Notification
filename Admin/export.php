	...
	if (isset($_POST['export_evaluation'])) {
      ...
      if (mysqli_num_rows($res) > 0) {
        foreach ($res as $row) {
          ...
          $visitor = array_merge($visitor, array(array($id, $name, $section_Id, $subject_Id, $row['avg_grand_total'].'/100', $ratings, date("F d, Y", strtotime($row['date_evaluated'])))));
        }
      } else {
    ...
    $id = 0;
      ...
      if (mysqli_num_rows($res) > 0) {
        foreach ($res as $row) {
          ...
          $name = $evaluatedInstructorRow['evaluatedInstructor'];
          $visitor = array_merge($visitor, array(array($id, $student_eval_name, $name, $row['yr_level'].' - '.$row['section'], $row['name'], $row['avg_grand_total'].'/100', $ratings, date("F d, Y", strtotime($row['date_evaluated'])))));
        }
      } else {
        $_SESSION['message'] = "No record found in the database.";
        $_SESSION['text'] = "Please try again.";
  ...
  if (isset($_POST['export_dean_evaluators'])) {
      ...
      if (mysqli_num_rows($res) > 0) {
        foreach ($res as $row) {
          ...
          $name = $evaluatedInstructorRow['evaluatedInstructor'];
          $visitor = array_merge($visitor, array(array($id, $student_eval_name, $name, $row['avg_grand_total'].'/100', $ratings, date("F d, Y", strtotime($row['date_evaluated'])))));
        }
      } else {
        $_SESSION['message'] = "No record found in the database.";
        $_SESSION['text'] = "Please try again.";
